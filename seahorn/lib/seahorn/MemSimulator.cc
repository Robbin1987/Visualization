...
namespace seahorn
{
  ...
  struct MemSimVisitor : public InstVisitor<MemSimVisitor>
  {
    ...
    unsigned ptrSz () const {return m_sim.getDataLayout ().getPointerSizeInBits ();}
    unsigned storeSz (const llvm::Type *t) const
    {return m_sim.getDataLayout ().getTypeStoreSize (const_cast<Type*> (t));}
    unsigned storeSz (const llvm::Value *v) const
    {return storeSz (v->getType ());}
    
    void add (Expr c) {m_side.push_back (c);}
    void addEq (Expr lhs, Expr rhs) {if (lhs && rhs) add (mk<EQ> (lhs, rhs));}
    /// add value v to equivalence class c
    void addEquiv (Expr v, Expr c)
    {
      if (!v || !c) return;
      if (c == m_largestPtr) return;
      Expr &u = m_equiv [c];
      if (u) addEq (v, u);
      else u = v;
    }
    ...
    {
      if (diff == 0) addEq (gep, base);
      else
        add (mk<EQ> (mk<BSUB> (gep, base),
                     bv::bvnum (diff, ptrSz (), efac ())));
    }
    
    Expr symb (const Value &V)
    {return m_sim.trace ().symb (m_loc, V);}
    ...
    {
      if (!I.getOperand (0)->getType ()->isPointerTy ()) return;
      LOG ("memsim",
           errs () << "  " << I << "\n";
      ...
      if (res)
      {
        ...
        if (isOpX<TRUE> (val)) add (res);
        else if (isOpX<FALSE> (val)) add (boolop::lneg (res));
      }
    ...
    void visitSelectInst (SelectInst &I)
    {
      ...
      Value *val = nullptr;
      if (isOpX<TRUE> (cVal)) val = I.getTrueValue ();
      else if (isOpX<FALSE> (cVal)) val = I.getFalseValue ();
      
      if (val) addEq (symb (I), symb (*val));
    ...
    void visitCallSite (CallSite CS)
    {
      ...
      if (llvm::isAllocationFn (CS.getInstruction (),
                                &m_sim.getTargetLibraryInfo (), true))
      {
        ...
        else
        {
          ...
          // -- TODO: extract symbolic definition of size and evaluate in the model
          assert (false && "NOT IMPLEMENTED");
        }
    ...
    void visitStoreInst (StoreInst &I)
    {
      ...
      // ptr + storeSz <= end (oid (ptr))
      Expr sz = bv::bvnum (storeSz (I.getValueOperand ()->getType ()), ptrSz(), efac());
      add (mk<BULE> (mk<BADD> (symb (*ptr), sz), endE (symb (*ptr))));
    }      
  ...
  bool MemSimulator::simulate ()
  {
    ...
    if (m_trace.size () == 0) return false;
    ...
    for (auto v : side) solver.assertExpr (v);
    auto res = solver.solve ();
    if (res)
    {
      LOG ("memsim",
           errs () << "Memory simulation: Success\n";);
    ...
    {
      LOG ("memsim",
           errs () << "Memory simulation: Failure\n";);
    ...
    {
      LOG ("memsim",
           errs () << "Memory simulation: Unknown!\n";);
      return false;
    }
  }
  
  Expr MemSimulator::eval (unsigned loc,
                           const llvm::Instruction &inst, bool complete)
  {
