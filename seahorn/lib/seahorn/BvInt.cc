...
namespace 
{
  struct BvIntRewriter : public std::unary_function<Expr,Expr>
  {
    ...
    {
      if (isOpX<BULT> (exp)) return mk<LT> (exp->left (), exp->right ());
      /// XXX handle special cases
      if (isOpX<BSLT> (exp)) return mk<LT> (exp->left (), exp->right ());
      if (isOpX<BULE> (exp)) return mk<LEQ> (exp->left (), exp->right ());
      if (isOpX<BSLE> (exp)) return mk<LEQ> (exp->left (), exp->right ());
      
      if (isOpX<BUGT> (exp)) return mk<GT> (exp->left (), exp->right ());
      if (isOpX<BSGT> (exp)) return mk<GT> (exp->left (), exp->right ());
      if (isOpX<BUGE> (exp)) return mk<GEQ> (exp->left (), exp->right ());
      if (isOpX<BSGE> (exp)) return mk<GEQ> (exp->left (), exp->right ());
     
      if (isOpX<BADD> (exp)) return mk<PLUS> (exp->left (), exp->right ());
      if (isOpX<BSUB> (exp)) return mk<MINUS> (exp->left (), exp->right ());
      if (isOpX<BMUL> (exp)) return mk<MULT> (exp->left (), exp->right ());
      if (isOpX<BUDIV> (exp)) return mk<IDIV> (exp->left (), exp->right ());
      if (isOpX<BSDIV> (exp)) return mk<IDIV> (exp->left (), exp->right ());
      if (isOpX<BUREM> (exp)) return mk<REM> (exp->left (), exp->right ());
      if (isOpX<BSREM> (exp)) return mk<REM> (exp->left (), exp->right ());
      if (isOpX<BSMOD> (exp)) return mk<MOD> (exp->left (), exp->right ());
      
      if (isOpX<BEXTRACT> (exp)) return bv::earg (exp);
      if (isOpX<BSEXT> (exp)) return exp->left ();
      if (isOpX<BZEXT> (exp)) return exp->left ();

      if (isOpX<BV2INT> (exp)) return exp->left ();
  ...
  struct BVINTABS : public std::unary_function<Expr, VisitAction>
  {
    ...
    {
      /// bv numeral
      if (bv::is_bvnum (exp)) return VisitAction::changeTo (exp->arg (0));
      ...
      if (bind::isFapp (exp) && exp->arity () == 1)
      {
        ...
        if (isOpX<BVSORT> (rangeTy))
          return VisitAction::changeTo (bind::intConst (abs::iabs (bind::fname (fdecl))));
        ...
        if (isOpX<ARRAY_TY> (rangeTy))
        {
          ...
          return VisitAction::changeTo (bind::mkConst (bind::fname (fdecl), rangeTy));
        }
        
        return VisitAction::skipKids ();
      }

      if (isOpX<INT2BV> (exp)) return VisitAction::changeTo (exp->left ());
