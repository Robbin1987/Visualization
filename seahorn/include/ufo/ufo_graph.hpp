#ifndef __UFO__GRAPH__HPP_
#define __UFO__GRAPH__HPP_
...
{  
	      ...
	      BoolVertexMap out)
  {
    ...
    if (reachExit.count (en) <= 0) return;
		  ...
		  BoolVertexMap out)
  {
    ...
    put (out, n, true);
    foreach (Edge edg, out_edges (n, g)) reachable (g, target (edg, g), out);
  } 
  ...
  bool isRoot (typename graph_traits<Graph>::vertex_descriptor v, 
	       const Graph &g)
  {
   ...
   */
  template<typename Graph>
  bool isLeaf (typename graph_traits<Graph>::vertex_descriptor v, 
	       const Graph &g)
  {
