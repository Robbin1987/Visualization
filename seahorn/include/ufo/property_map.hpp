#ifndef __UFO__PROPERTY_MAP_HPP_
#define __UFO__PROPERTY_MAP_HPP_

/** Helper classes to use Property Maps */

namespace ufo
{
  template<typename Set>
  struct SetPropertyMap 
  {
  ...
  template<typename Set>
  SetPropertyMap<Set> make_set_property_map (Set &s) 
  {
...
{  
  ...
  {
    assert (pm.set != NULL);
    if (val) pm.set->insert (key); else pm.set->erase (key);
  }
}
